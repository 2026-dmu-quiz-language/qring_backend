...
public interface CompetitionWrongAnswerRepository extends JpaRepository<CompetitionWrongAnswer, Long> {
    ...
    // 레벨별 중복 없이, 가장 최근 오답 시각 포함 조회 (7일 이내, 현재 학습 언어만) — STORY/COMPETITION 통합 목록 정렬용
    @Query("SELECT new com.qring.qring_backend.dto.quiz.IncorrectResponseDto$WrongAnswerItem(" +
           "'COMPETITION', CAST(wa.level AS long), CONCAT('레벨 ', wa.level, ' 컴피티션'), wa.level, MAX(wa.createdAt)) " +
    ...
    // 레벨로 오답 문제 목록 조회 (재풀이용, 7일 이내, 현재 학습 언어만) — 원본 문제 조회는 IncorrectService 에서 sourceType 별로 분기
    @Query("""
        SELECT wa FROM CompetitionWrongAnswer wa
        WHERE wa.userId = :userId AND wa.level = :level
        AND wa.langCode = :langCode
        AND wa.createdAt >= :cutoff
    """)
    List<CompetitionWrongAnswer> findByUserIdAndLevelAndLangCode(@Param("userId") Long userId,
